...
/* ------------------------------------------------------------------ *
 * THE CAVE — Player Board (two-screen prototype)
 * Phone = Controller (type names) -> TV = Display (live scoreboard)
 * Sync runs through Claude's built-in artifact storage (shared key),
 * polled every couple of seconds. Works while both screens are open
 * inside Claude. No localStorage/sessionStorage used.
 * ------------------------------------------------------------------ */
...
function useBoard(active) {
  ...
  const [status, setStatus] = useState(hasStorage() ? "connecting" : "local");
  const [lastSynced, setLastSynced] = useState(null);
  const localWriteAt = useRef(0); // controller stamps its own edits so polls don't clobber typing
    ...
    async function read() {
      ...
      try {
        ...
        if (res && res.value) {
          ...
          if (remote && typeof remote.updatedAt === "number" && remote.updatedAt >= localWriteAt.current) {
            setBoard(remote);
          }
        }
        setStatus("live");
  ...
  const setPlayer = useCallback(
    (arenaKey, idx, name) => {
      setBoard((prev) => {
        ...
        localWriteAt.current = next.updatedAt;
        persist(next);
        return next;
      });
    },
